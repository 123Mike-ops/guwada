                                  ...
                                  <table class="table table-borderless table-striped table-earning" >
                                      ...
                                      <tbody>
                                                    <tr>
                                                        ...
                                                        @endforeach
                                                        <tr><td class="text-center"><a class="btn btn-primary" href="{{route('Bookcheck.cook',['bookId'=>$bookId,'restId'=>$restId])}}"style="color:white;">Finish Task</a></td></tr>
                                                        @endif
                                            @if($type)
                                                    <td class="text-center">{{$orderId}}</td>
                                                    @foreach($items as $item)
                                                    ...
                                                    <td class="text-center"><img src="{{ asset('uploads/FoodType/'.$types->image)}}" style="height:50px;"class="img-fluid" alt="Image"></td>
                                                        ...
                                                        <td class="text-center">
                                                                <table>
                                                                    ...
                                                                    @if(!($selectedIngrident['orderStarted']))
                                                                    <td><a class="btn btn-warning" href="{{route('UseIngrident',['id'=>$selectedIngrident['id']])}}">use</a></td>
                                                                    @else
                                                                    <td><a class="btn btn-success" style="color:white" >used</a></td>
                                                                    @endif
